...
import { Home, Award, TrendingUp, Clock, Flame, ArrowUp, Download, Heart, Activity } from 'lucide-react';
import { getIcon } from '../utils/iconMap';

export default function WorkoutSummaryModal({ summary, onClose, workoutTitle }) {
    // Detect cardio vs strength
    const isCardio = workoutTitle?.startsWith('Cardio:') || workoutTitle?.startsWith('Cardio ');

    // Theme based on type
    const confettiCount = 30;
    const confettiColors = isCardio
        ? ['#F97316', '#FB923C', '#FBBF24']
        : ['#A855F7', '#34D399', '#FBBF24'];

    const accentColor = isCardio ? 'orange' : 'purple';
    const BadgeIcon = isCardio ? Heart : Award;
    ...
    return (
        <div className="fixed inset-0 z-[60] flex flex-col bg-[#0B0B0F]/95 backdrop-blur-md animate-in fade-in duration-200 overflow-hidden">
            ...
            {/* Background Effects */}
            <div className={`absolute inset-0 bg-[radial-gradient(circle_at_top,_var(--tw-gradient-stops))] ${isCardio ? 'from-orange-900/20' : 'from-purple-900/20'} via-[#0B0B0F]/50 to-[#0B0B0F] pointer-events-none`}></div>
            ...
            <style>{`
                ...
                .neon-text-glow {
                    text-shadow: 0 0 10px ${isCardio ? 'rgba(249, 115, 22, 0.5), 0 0 20px rgba(249, 115, 22, 0.3)' : 'rgba(168, 85, 247, 0.5), 0 0 20px rgba(168, 85, 247, 0.3)'};
                }
                .gold-badge-glow {
            ...
            <div className="fixed top-0 left-0 w-full h-full overflow-hidden -z-10 pointer-events-none">
                <div className={`absolute top-[-10%] left-1/2 -translate-x-1/2 w-[60%] h-[40%] ${isCardio ? 'bg-orange-600/20' : 'bg-purple-600/20'} blur-[150px] rounded-full animate-pulse-slow`}></div>
                <div className={`absolute bottom-[-10%] right-[-10%] w-[40%] h-[40%] ${isCardio ? 'bg-rose-500/10' : 'bg-green-500/10'} blur-[120px] rounded-full`}></div>
            </div>
            ...
            <main className="modal-enter w-full max-w-4xl mx-auto flex flex-col gap-6 md:gap-8 relative z-10 p-4 md:pt-12 h-full overflow-y-auto custom-scrollbar">
                ...
                <header className="text-center relative shrink-0 pt-4 md:pt-0">
                    <div className={`inline-flex items-center justify-center w-20 h-20 md:w-24 md:h-24 rounded-full ${isCardio ? 'bg-gradient-to-b from-orange-300 to-orange-600 border-orange-200 shadow-orange-400/30' : 'bg-gradient-to-b from-yellow-300 to-yellow-600 border-yellow-200 shadow-yellow-400/30'} text-white mb-4 md:mb-6 shadow-lg animate-float border-2 gold-badge-glow`}>
                        <BadgeIcon size={48} className="md:w-[3.5rem] md:h-[3.5rem] text-white fill-current" />
                    </div>
                    <h1 className="text-3xl md:text-5xl font-black text-white mb-2 tracking-tight neon-text-glow uppercase">
                        {isCardio ? 'Cardio Finalizado!' : 'Treino Finalizado!'}
                    </h1>
                    <p className="text-slate-400 text-sm md:text-lg font-medium max-w-lg mx-auto">
                        {isCardio ? 'Sessão de cardio concluída! Seu coração agradece. 💪' : 'Você superou seus limites hoje! Ótimo trabalho.'}
                    </p>
                </header>

                <div className={`relative w-full max-w-2xl mx-auto p-[1px] rounded-xl bg-gradient-to-r ${isCardio ? 'from-orange-500/20 via-orange-500 to-orange-500/20' : 'from-purple-500/20 via-purple-500 to-purple-500/20'} shadow-glow-message shrink-0`}>
                    <div className="relative bg-[#13131A]/90 backdrop-blur-md rounded-xl p-4 md:p-6 text-center border border-white/5">
                        <p className="text-base md:text-xl text-white font-medium leading-relaxed">
                            Parabéns! Você completou <span className={`font-extrabold ${isCardio ? 'text-orange-400' : 'text-[#34D399]'}`}>{workoutTitle}</span> e ficou <span className={`font-extrabold ${isCardio ? 'text-orange-400' : 'text-[#34D399]'}`}>{isCardio ? 'mais resistente' : 'mais forte'}</span> hoje! 🔥
                        </p>
                    </div>
                </div>

                <div className={`grid grid-cols-1 ${isCardio ? 'md:grid-cols-2' : 'md:grid-cols-3'} gap-3 md:gap-6 w-full shrink-0`}>
                    {/* Time */}
                    ...
                    {/* PRs - only for strength workouts */}
                    {!isCardio && (
